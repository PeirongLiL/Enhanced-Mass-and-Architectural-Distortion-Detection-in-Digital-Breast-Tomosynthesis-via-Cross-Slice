...
def evaluate(
    ...
    # df_boxes = df_boxes[df_boxes['AD'] == 1]
...
def _froc(
    ...
    n_bootstrap: int = 500,  # 新增参数，控制 bootstrap 的次数
    ...
    for th in sorted(thresholds, reverse=True):
        ...
        tpr_th = n_tps_th / len(df_annotations)  # 使用 df_annotations 总行数代替 n_boxes
    ...
    fps_at_90_sensitivity = None

    # 确保我们有足够的数据点进行插值
    if len(tpr) >= 2 and min(tpr) <= target_sensitivity <= max(tpr):
        # 我们需要反转x和y，因为我们要从敏感度找FPS
        fps_at_90_sensitivity = np.interp(target_sensitivity, tpr, fps)
    elif max(tpr) < target_sensitivity:
        # 如果最大敏感度仍低于90%，返回None或最大FPS
        fps_at_90_sensitivity = None
    else:
        # 如果最小敏感度已高于90%，返回最小FPS
        fps_at_90_sensitivity = min(fps)
    print("90%sensitive fps:",fps_at_90_sensitivity)
    # 绘制FROC曲线，限制X轴范围在0到4
