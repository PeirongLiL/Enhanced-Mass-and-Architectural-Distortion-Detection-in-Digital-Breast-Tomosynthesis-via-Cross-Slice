...
df = read_boxes(boxes_fp="data/boxes-test.csv", filepaths_fp="data/file-paths-test.csv")#读取了box和地址参数，并一起返回


# print(df)
output_path = r'D:\pycharm project\dataset\dbt duke\manifest-1617905855234/sample2/'
output_folder = 'test'
# create folders
try:
...
def copy_case_rgb_multi(view, client, episode, image_rgb, side, bbox_list, slice_index):
    ...
    if (side == 'r' or side == 'R'):  # 如果 side 为 'r' 或 'R'，将图像水平翻转，并调整ROI的坐标。
        aux = bbox.x2
        bbox.x2 = dims[1] - bbox.x1
        bbox.x1 = dims[1] - aux
    with open(output_path + '/test_bboxes3.csv', 'a+', newline='') as file:
...
for group_key, group_df in grouped:
    client, episode, view, _ = group_key
    ...
    for slice_index in range(6, int(group_df.iloc[0]["VolumeSlices"]) - 6):
